...
def Create_Settings_Menu_Window(label):
    if (label == lang['Header']['Edit'][2]) and not isWinOpen:
        ...
        optionmenu_theme_var = StringVar(value=settings['default_theme'])
...
class Additional():
    ...
    def save_sets(var_lang, var_theme):
        ...
        settings['default_theme'] = var_theme
    ...
    def optionmenu_themes_callback(var):
        if var == 'Светлая' or var == 'Light':
            set_appearance_mode('light')
        elif var == 'Темная' or var == 'Dark':
            set_appearance_mode('dark')
        elif var == 'Bern' or var == 'Берн':
            set_appearance_mode('dark')
            set_default_color_theme("bern_app.json") 
        elif var == 'Lambda' or var == 'Лямбда':
            set_appearance_mode('light')
